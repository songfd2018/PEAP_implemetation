...
load("peap_final_pairs.RData")
load("Nonoverlap_top100GenesOfEachGenepairs.Rdata")
load("Gene_expression_matrix_20240228.Rdata")

peap_final_pairs = as.data.frame(peap_final_pairs)
marker_gene_pairs = paste0(peap_final_pairs[,1], "_", peap_final_pairs[,2])
gene_pair_ind = paste0(top100_Genes[,1], "_", top100_Genes[,2])

marker_gene_pairs_ind = match(marker_gene_pairs, gene_pair_ind)

setwd("D:/PEAP/PEAP_02/gene_expression/")
...
for(j in marker_gene_pairs_ind){
  filename = which(names(KEGG_list)==top100_Genes[j,5])
  load(paste0("Gene_expression_KEGG",filename,"_v8.RData"))
...
PEAP_all = cbind(PEAP_res_std, PEAP_res_pos[,3:12], PEAP_res_neg[,3:12])

for(i in 1:nrow(PEAP_all)){
  for(j in 1:5){
    if(is.na(PEAP_all[i,j+7])){
      if(PEAP_all[i,j+2]>0){
        PEAP_all[i,j+7] = PEAP_all[i,j+17]
      }
      else(PEAP_all[i,j+7] = PEAP_all[i,j+27])
    }
  }
  
}

PEAP_all = PEAP_all[,1:12]
colnames(PEAP_all) = c("gene1", "gene2", "ES.Basal", "ES.Her2", "ES.LumA", "ES.LumB", "ES.Normal", "pvalue.Basal", "pvalue.Her2", "pvalue.LumA", "pvalue.LumB", "pvalue.Normal")
write.csv(PEAP_all, file = "PEAP_res_BRCA_20240424.csv",row.names = FALSE)
